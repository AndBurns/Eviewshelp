...
' Electricity data 
	...
	' Average electricity price (USD cents/kwh): https://govdata360.worldbank.org/indicators/h6779690b?country=BGD&indicator=42573&countries=BRA&viz=line_chart&years=2014,2019
...
' CREATE IO SCALARS AND MAPPING
	...
	importtbl(name=io) %pathxlsdata2 range=EviewsIO na="#N/A"
...
' https://support.sas.com/rnd/app/ets/examples/translog_elasticities/index.pdf
	...
	equation _{%cty}NVAGRTOT.ls BGDNVAGRTOT =C(2)*LOG(BGDNVAGRTOTLXN/BGDNVSRVTOTLXN)+C(3)*LOG(BGDNVINDOTHRXN/BGDNVSRVTOTLXN)+C(4)*LOG(BGDNVENGTOTLXN/BGDNVSRVTOTLXN)+C(5)*BGDNVAGRTOT(-1)+(1-C(5))*(BGDNVAGRSMKN/BGDNYGDPFCSTKN) '  +C(1)

	' Price elasticity of demand 
	'	for i!=j: c(3)/{%cty}NVAGRTOT+{%cty}NVOTHRETOT [cross price elasticity]
	'	for i=j: c(2)/{%cty}NVAGRTOT+{%cty}NVAGRTOT	-1 [own price elasticity]
	'	for i!=j: (c(3)/({%cty}NVOTHRETOT*{%cty}NVAGRTOT)) + 1 [Hicks-Allen (Allen-Uzawa) elasticity of substitution or sigma]
	...
	if !SR1 = 1 then
		...
		equation _{%cty}NVENETOT.ls BGDNVENETOT=C(1)+C(2)*LOG(BGDNVAGRTOTLXN/BGDNVSRVTOTLXN)+C(3)*LOG(BGDNVINDOTHRXN/BGDNVSRVTOTLXN)-0.06*LOG(BGDNVENGTOTLXN/BGDNVSRVTOTLXN)+0.006*LOG(BGDNVENESMKN)+C(10)*@TREND*@DURING("2009 2018")
	endif
	...
	equation _{%cty}NVOTHRETOT.ls BGDNVOTHRETOT=C(2)*LOG(BGDNVAGRTOTLXN/BGDNVSRVTOTLXN)+C(3)*LOG(BGDNVINDOTHRXN/BGDNVSRVTOTLXN)+C(4)*LOG(BGDNVENGTOTLXN/BGDNVSRVTOTLXN)+c(5)*BGDNVOTHRETOT(-1)+(1-c(5))*(BGDNVINDSMKN/BGDNYGDPFCSTKN)+c(1)'+c(10)*t_lr ' +C(1)
	equation _{%cty}NVOTHRETOT.ls BGDNVOTHRETOT=0.02*LOG(BGDNVAGRTOTLXN/BGDNVSRVTOTLXN)+-0.1*LOG(BGDNVINDOTHRXN/BGDNVSRVTOTLXN)-0.03*LOG(BGDNVENGTOTLXN/BGDNVSRVTOTLXN)+(1)*(BGDNVINDSMKN/BGDNYGDPFCSTKN)+C(1)*@DURING("2016")+C(10)*@DURING("2000 2007")
...
' Electricity - consumption commodities 
	...
	for %ah {fuelsprod} ' C(k) = Y(k)+M(k) - T(k)
		...
		if %ah = "REN" then
			series {%cty}NE{%ah}CONNEXN = (0.001+{%cty}NE{%ah}CONNECN)/(0.001+{%cty}NE{%ah}CONNEKN)  ' Non-Electricity consumption of commodity k (real)
		else
...
' INDUSTRY PRICES
	...
	for %vv {VAS}
		...
		equation _{%cty}NV{%vv}TOTLXN.LS(OPTMETHOD=LEGACY) DLOG({%cty}NV{%vv}TOTLXN)=-0.2*(LOG({%cty}NV{%vv}TOTLXN(-1))-LOG({%cty}NYGDPFCSTXN(-1)))+DLOG({%cty}NYGDPFCSTXN)+ C(10)
	next
	...
	equation _{%cty}NVINDOTHRXN.LS(OPTMETHOD=LEGACY) DLOG({%cty}NVINDOTHRXN)=-0.2*(LOG({%cty}NVINDOTHRXN(-1))-LOG({%cty}NYGDPFCSTXN(-1)))+1*DLOG({%cty}NYGDPFCSTXN)+C(10)
...
'***************************************************************************
	...
	delete _{%cty}NVCOLPRODKN
	{%cty}.append @identity {%cty}NVCOLPRODKN = {%cty}NVENGTOTLKN-{%cty}NVRENPRODKN-{%cty}NVOILPRODKN-{%cty}NVGASPRODKN
	...
	{%cty}.append @identity BGDNVRENPRODKN =  BGDNERENCONKN - BGDNVRENNIMPKN 
	{%cty}.append @identity BGDNVRENPRODCN = BGDNERENCONCN -  BGDNVRENNIMPCN 

' VOLUMES AND EMISSIONS
' CONSUMPTION CES (ENERGY; OTHER)

if !SR1 = 1 then
	equation _{%cty}NECONENGYKN.LS DLOG({%cty}NECONENGYKN) = C(1)-0.3*(LOG({%cty}NECONENGYKN(-1))-LOG({%cty}NECONPRVTKN(-1))-0.3*LOG({%cty}NECONPRVTXN(-1)/{%cty}NECONENGYXN(-1)))+0.3*DLOG({%cty}NECONPRVTKN)-0.3*DLOG({%cty}NECONPRVTXN/{%cty}NECONENGYXN)  ' Consumption Energy CES
...
if !SR1 = 1 then
	equation _{%cty}NECONELEKN.LS DLOG({%cty}NECONELEKN) = C(1)-0.3*(LOG({%cty}NECONELEKN)-LOG({%cty}NECONENGYKN(-1))-0.3*LOG({%cty}NECONENGYXN(-1)/{%cty}NECONELEXN(-1)))+0.3*DLOG({%cty}NECONENGYKN)-0.3*DLOG({%cty}NECONENGYXN/{%cty}NECONELEXN)    ' Consumption Energy CES
...
' Consumption non-electricity prices of commodity k
	...
	''equation _{%cty}NECONOTHRXN.LS DLOG(BGDNECONOTHRXN) = -0.3*(LOG(BGDNECONOTHRXN(-1))-0.5*LOG(BGDNYGDPFCSTXN(-1))-LOG(1+BGDGGREVTVATER(-1)/100)-(1-0.5)*LOG(BGDNEIMPGNFSXN(-1)))+C(3)+0.3*DLOG(BGDNEIMPGNFSXN)+(1-0.3)*DLOG(BGDNYGDPFCSTXN)+DLOG(1+BGDGGREVTVATER/100) +C(10)*@DURING("2009 2010") +0.3*BGDNYGDPGAP_/100
...
'Values and volumes
	...
	{%cty}.append @identity {%cty}NVCOLPRODXN = {%cty}NVCOLPRODCN/{%cty}NVCOLPRODKN
...
'***************************************************************************
	...
	equation _{%cty}LMWAPOPKN.LS DLOG(BGDLMWAPOPKN) = C(1)-0.3*(LOG(BGDLMWAPOPKN(-1))-LOG(BGDSPPOP1564TO(-1)*(1-BGDWDL(-1)/(BGDSPPOP1564TO*240))))+DLOG(BGDSPPOP1564TO*(1-BGDWDL/(BGDSPPOP1564TO*240)))
