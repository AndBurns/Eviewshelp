...
'Exclusion for row #18, Var:NEGDISTKBKN
...
smpl @all
series BGDNEGDISTKBKN_2 = BGDNEGDISTKBKN
smpl %fcst_start %fcst_end

{%model}.exclude(r) BGDNEGDISTKBKN_A

'Mode set to contr: setting shock value (_2) of series BGDNEGDISTKBKN equal to level consistent with contribution to GDP growth recorded in excel sheet (hold) 
series BGDNEGDISTKBKN_2 =BGDNEGDISTKBKN_2(-1) + ((hold / 100))* BGDNYGDPMKTPKN(-1)
{%model}.override(m) BGDNEGDISTKBKN
{%model}.exclude(m) BGDNEGDISTKBKN
{%model}.exclude(r) BGDNEEXPGNFSKN_A
{%model}.exclude(r) BGDNEIMPGNFSKN_A



'Exclusion for row #29, Var:NYGDPDISCKN
'Placing raw data from excel in hold variable for later treatment, and create the shock variable (_2)

hold.fill(o=2023)   0, 0, 0, 0, 0, 0, 0, 0
smpl @all
series BGDNYGDPDISCKN_2 = BGDNYGDPDISCKN
smpl %fcst_start %fcst_end

{%model}.exclude(r) BGDNYGDPDISCKN_A

'Mode set to contr: setting shock value (_2) of series BGDNYGDPDISCKN equal to level consistent with contribution to GDP growth recorded in excel sheet (hold) 
series BGDNYGDPDISCKN_2 =BGDNYGDPDISCKN_2(-1) + ((hold / 100))* BGDNYGDPMKTPKN(-1)
{%model}.override(m) BGDNYGDPDISCKN
{%model}.exclude(m) BGDNYGDPDISCKN
'The following variables were in exogenous mode but were ucnhanged
{%model}.exclude(m) BGDNECONPRVTKN BGDNEGDIFPRVKN BGDNEEXPGNFSKN BGDNEIMPGNFSKN BGDNYGDPTFP
...
'Exclusion for row #10, Var:NYTAXNINDKN
'Placing raw data from excel in hold variable for later treatment, and create the shock variable (_2)

hold.fill(o=2023)   6.88835757898176, 7.55569549437169, 7.65596663373349, 7.04332452007195, 6.69403418981622, 6.56866700572183, 6.56331782865336, 6.60659017403498
smpl @all
series BGDNYTAXNINDKN_2 = BGDNYTAXNINDKN
smpl %fcst_start %fcst_end

{%model}.exclude(r) BGDNYTAXNINDKN_A

'Mode set to growth: setting shock value (_2) of series BGDNYTAXNINDKN equal to level consistent with growth rates recorded in excel sheet (hold) 
series BGDNYTAXNINDKN_2 = BGDNYTAXNINDKN_2(-1) * (1 + hold / 100)
{%model}.override(m) BGDNYTAXNINDKN
{%model}.exclude(m) BGDNYTAXNINDKN
{%model}.exclude(r) BGDNYTAXNINDKN_A
...
'Exclusion for row #46, Var:NYTAXNINDCN
'Placing raw data from excel in hold variable for later treatment, and create the shock variable (_2)

hold.fill(o=2023)   12.3819448557425, 13.2353437261513, 13.7484000869533, 13.591832753157, 13.765924859991, 13.9191701179781, 14.0018778101597, 14.0197914861936
smpl @all
series BGDNYTAXNINDCN_2 = BGDNYTAXNINDCN
smpl %fcst_start %fcst_end

{%model}.exclude(r) BGDNYTAXNINDCN_A

'Mode set to growth: setting shock value (_2) of series BGDNYTAXNINDCN equal to level consistent with growth rates recorded in excel sheet (hold) 
series BGDNYTAXNINDCN_2 = BGDNYTAXNINDCN_2(-1) * (1 + hold / 100)
{%model}.override(m) BGDNYTAXNINDCN
{%model}.exclude(m) BGDNYTAXNINDCN
{%model}.exclude(r) BGDNYTAXNINDCN_A
'The following variables were in exogenous mode but were ucnhanged
{%model}.exclude(m) BGDNVAGRTOTLKN BGDNVINDTOTLKN BGDNYGDPCPSHXN BGDNVAGRTOTLXN BGDNVINDTOTLXN
...
'The following variables were in exogenous mode but were ucnhanged
{%model}.exclude(m) BGDBXGSRMRCHCD BGDBMGSRMRCHCD BGDBXFSTREMTCD BGDBXFSTOTHRCD BGDBMFSTREMTCD BGDBMFSTOTHRCD BGDBFCAFFFDICD BGDBFCAFFPFTCD BGDBFCAFOOTHCD BGDBFCAFCAPTCD BGDBFCAFNEOMCD BGDBFCAFRACGCD
...
{%model}.exclude(r) BGDGGREVTOTHCN_A
{%model}.exclude(r) BGDGGREVNONTCN_A
{%model}.exclude(r) BGDGGREVGRNTCN_A
'The following variables were in exogenous mode but were ucnhanged
{%model}.exclude(m) BGDGGREVDRCTCN BGDGGREVTVATCN BGDGGREVIMPDCN BGDGGREVEXPDCN BGDGGREVEXCDCN BGDGGREVSUPDCN BGDGGREVOTHDCN BGDGGREVNNBRCN BGDGGREVTOTHCN BGDGGREVOTHRCN BGDGGREVNONTCN BGDGGREVGRNTCN
...
%Fname="C:\WBG\LocalITUtilities\BGD\Data\BGD2024-01-02~11-46-44soln.wf1"
wfsave(2) %Fname
close BGD2024-01-02~11-46-44soln
wfselect BGDsoln
