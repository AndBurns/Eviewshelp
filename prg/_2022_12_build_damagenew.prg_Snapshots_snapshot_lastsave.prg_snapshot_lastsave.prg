...
'====================================
	...
	smpl @first @first
	series {%cty}NEADPKSTGKN = 0 ' 0.00001 ' Adaptation capital
	smpl @first+1 %end_dat
...
series {%cty}NEFLOPTVTKN = @recode(BGDNEADPKSTGKN=0,0,1.0*({%cty}NEADPKSTGKN/{%cty}NEADPKMAXKN)^0.3) '
	...
	smpl %stDate+1 %end_dat
	{%cty}NEGDIKSTKKN2 = {%cty}NEGDIKSTKKN2(-1) * (1 - {%cty}DEPR/100) + {%cty}NEGDIFTOTKN-{%cty}RECOVINVESTKN
' ============================
...
' ============================
	...
	{%cty}.append @IDENTITY {%cty}NEGDIKSTKKN2 = {%cty}NEGDIKSTKKN2(-1) * (1 - {%cty}DEPR/100) + {%cty}NEGDIFTOTKN-{%cty}RECOVINVESTKN
...
'***************************************** 
	...
	{%cty}.append @identity {%cty}NEFLOPTVTKN = @recode(BGDNEADPKSTGKN=0,0,1.0*({%cty}NEADPKSTGKN/{%cty}NEADPKMAXKN)^0.3) '
...
' Change fiscal estimates
	...
	smpl @first 2018
	!now = 0.15
	''equation _{%cty}GGEXPGFCICN.LS(OPTMETHOD=LEGACY) (BGDGGEXPGFCICN/(BGDGGREVTOTLCN-BGDGGEXPINTPCN(-1))) =  !now*{%cty}GGEXPGFCICN(-1)+(1-!now)*(0.09 *({%cty}GGREVTOTLCN-{%cty}GGEXPINTPCN(-1))) - (1-{%cty}fininv)*({%cty}DISPREPCN) +C(10) *@DURING("2012")
	''equation _{%cty}GGEXPGNFSCN.LS(OPTMETHOD=LEGACY) (BGDGGEXPGNFSCN/(BGDGGREVTOTLCN-BGDGGEXPINTPCN(-1))) =  !now*{%cty}GGEXPGNFSCN(-1)+(1-!now)*(0.36 *({%cty}GGREVTOTLCN-{%cty}GGEXPINTPCN(-1))) - (1-{%cty}fininv)*({%cty}DISPREPCN) +C(10) *@DURING("2012")
	...
	smpl @all
	series {%cty}GGEXPOTHRCN = {%cty}GGEXPTOTLCN - (BGDGGEXPCRNTCN  + BGDGGEXPCAPTCN  + BGDHEAPCN+{%cty}DISPREPCN+{%cty}DISPREPCNI+{%cty}DISPREPCNC)
	equation _{%cty}GGEXPOTHRCN.LS {%cty}GGEXPOTHRCN/({%cty}GGREVTOTLCN-{%cty}GGEXPINTPCN(-1)) = C(1)

	{%cty}.append @IDENTITY {%cty}GGEXPTOTLCN = BGDGGEXPCRNTCN  + BGDGGEXPCAPTCN  + BGDGGEXPOTHRCN  + BGDHEAPCN +{%cty}DISPREPCN+{%cty}DISPREPCNI+{%cty}DISPREPCNC
	{%cty}.append @IDENTITY {%cty}GGREVTOTLCN = BGDGGREVTAXTCN  + BGDGGREVNONTCN  + BGDGGREVGRNTCN  + BGDGGREVOTHRCN  + BGDGGREVEMISCN  + BGDGGREVELECN+({%cty}insu+{%cty}cofu)*{%cty}PAYOUTCN
