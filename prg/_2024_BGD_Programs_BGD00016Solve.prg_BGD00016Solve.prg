...
'Exclusion for row #7, Var:GGREVDRCTCN
'Placing raw data from excel in hold variable for later treatment, and create the shock variable (_2)

hold.fill(o=2023)   2.42013755428694, 3.4, 3.4, 3.4, 3.4, 3.4, 3.4, 3.4
smpl @all
series BGDGGREVDRCTCN_2 = BGDGGREVDRCTCN
smpl %fcst_start %fcst_end

{%model}.exclude(r) BGDGGREVDRCTCN_A

'Mode set to %gdp: setting shock value (_2) of series BGDGGREVDRCTCN equal to level consistent with percent of GDP recorded in excel sheet (hold) 
series BGDGGREVDRCTCN_2 = (hold / 100) * BGDNYGDPMKTPCN
{%model}.override(m) BGDGGREVDRCTCN
{%model}.exclude(m) BGDGGREVDRCTCN
{%model}.exclude(r) BGDGGREVDRCTCN_A
...
%Fname="C:\WBG\LocalITUtilities\BGD\Data\BGD2024-05-02~11-12-17soln.wf1"
wfsave(2) %Fname
close BGD2024-05-02~11-12-17soln
wfselect BGDsoln
