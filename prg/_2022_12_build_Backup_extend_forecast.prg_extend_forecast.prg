...
'#################################################################
subroutine extend_forecast(string i_cty,string i_fcst_start, string i_fcst_end, string l_constant, string l_value, string l_volume, string l_price, string l_wage, string l_population, string l_dummy, scalar i_inflexpt, scalar i_popgrowth, scalar i_tfpgrowth, scalar i_labshare)
	...
	'Volume
		...
		if @wcount(%buff)>0 then
			for %var {%buff}
				smpl %buff_start %buff_end
				{%var} = {%var}(-1)*(1+i_tfpgrowth/100)^(1/i_labshare)*(1+i_popgrowth/100)
			next
		endif
	...
	'Value
		...
		if @wcount(%buff)>0 then
			for %var {%buff}
				smpl %buff_start %buff_end
				{%var} = {%var}(-1)*(1+i_tfpgrowth/100)^(1/i_labshare)*(1+i_popgrowth/100)*(1+i_inflexpt/100)
			next
		endif
