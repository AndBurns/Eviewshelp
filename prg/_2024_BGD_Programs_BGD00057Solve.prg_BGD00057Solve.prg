...
'Exclusion for row #14, Var:NECONGOVTXN
'Placing raw data from excel in hold variable for later treatment, and create the shock variable (_2)

hold.fill(o=2023)   -1.9, 2.1734826203047, 2.25278900992882, 2.82042455306835, 3.32317598651626, 3.86276516787987, 4.41416481574362, 4.97784611998091
smpl @all
series BGDNECONGOVTXN_2 = BGDNECONGOVTXN
smpl %fcst_start %fcst_end

{%model}.exclude(r) BGDNECONGOVTXN_A

'Mode set to growth: setting shock value (_2) of series BGDNECONGOVTXN equal to level consistent with growth rates recorded in excel sheet (hold) 
series BGDNECONGOVTXN_2 = BGDNECONGOVTXN_2(-1) * (1 + hold / 100)
{%model}.override(m) BGDNECONGOVTXN
{%model}.exclude(m) BGDNECONGOVTXN
{%model}.exclude(r) BGDNECONGOVTXN_A
...
'The following variables were in exogenous mode but were ucnhanged
{%model}.exclude(m) BGDFPCPITOTLXN BGDNECONPRVTXN BGDNEGDIFGOVXN BGDNEGDIFPRVXN BGDNEEXPGNFSXN BGDNEIMPGNFSXN BGDPANUSATLS BGDFMLBLPOLYFR
...
%Fname="C:\WBG\LocalITUtilities\BGD\Data\BGD2024-12-02~04-56-28soln.wf1"
wfsave(2) %Fname
close BGD2024-12-02~04-56-28soln
wfselect BGDsoln
