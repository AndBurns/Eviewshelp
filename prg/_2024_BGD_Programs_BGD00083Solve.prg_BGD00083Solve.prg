...
'Exclusion for row #33, Var:GGEXPCAPTCN
'Placing raw data from excel in hold variable for later treatment, and create the shock variable (_2)

hold.fill(o=2024)   10, 13.2584050968372, 13.0940675707292, 13.0582659713539, 13.1175859387844, 13.3206005850027, 13.6535305093622
smpl @all
series BGDGGEXPCAPTCN_2 = BGDGGEXPCAPTCN
smpl %fcst_start %fcst_end

{%model}.exclude(r) BGDGGEXPCAPTCN_A

'Mode set to growth: setting shock value (_2) of series BGDGGEXPCAPTCN equal to level consistent with growth rates recorded in excel sheet (hold) 
series BGDGGEXPCAPTCN_2 = BGDGGEXPCAPTCN_2(-1) * (1 + hold / 100)
{%model}.override(m) BGDGGEXPCAPTCN
{%model}.exclude(m) BGDGGEXPCAPTCN
{%model}.exclude(r) BGDGGEXPCAPTCN_A
'The following variables were in exogenous mode but were ucnhanged
{%model}.exclude(m) BGDGGEXPGNFSCN BGDGGEXPWAGECN BGDGGEXPTRNSCN BGDINTRDDIFFFR BGDINTREDIFFFR BGDUSAINTRFR BGDGGEXPOTHRCN
...
%Fname="C:\WBG\LocalITUtilities\BGD\Data\BGD2024-14-02~00-47-34soln.wf1"
wfsave(2) %Fname
close BGD2024-14-02~00-47-34soln
wfselect BGDsoln
