...
'*******************************************************
	...
	''equation _BGDLMEMPTOTL.LS DLOG(BGDLMEMPTOTL) =-0.2*(LOG(BGDLMEMPTOTL(-1))-LOG(BGDLMEMPSTRL(-1))) + DLOG(BGDLMEMPSTRL) - c(2)*(DLOG(BGDNYWRTTOTLXN/BGDNYGDPFCSTXN)-DLOG(BGDNYGDPPOTLKN/BGDLMEMPSTRL)) +c(3)*(DLOG(BGDNEGDETTOTKN)-DLOG(BGDNYGDPPOTLKN))+C(4)
...
''equation _BGDNYGDPFCSTXN.LS DLOG(BGDNYGDPFCSTXN) = C(1)*DLOG(BGDNYGDPFCSTXN(-1))+(1-C(1))*(0.7*(BGDINFLEXPT/100)+(1-0.7)*DLOG(BGDPSTAR(-1))) +0.35*BGDNYGDPGAP_/100
...
'				BGD.append @IDENTITY BGDGGREVTOTHCN = BGDGGREVTOTHER / 100 * BGDNYGDPMKTPCN
	...
	'EXPENDITURES
		...
		'WAGE
			smpl %begin_date %end_estimate
				c = 0
			equation _BGDGGEXPWAGECN.LS DLOG(BGDGGEXPWAGECN) = C(1) - C(2) * (LOG(BGDGGEXPWAGECN(-1)) - LOG(BGDNYGDPMKTPCN(-1))) +DLOG(BGDNYGDPMKTPCN) ' C(3) * DLOG(BGDGGEXPGNFSCN(-1)) + (1 - C(3))* DLOG(BGDNYGDPMKTPCN)
			

		'GOODS and SERVICES
			smpl 2000 %end_estimate
				c = 0		
			equation _BGDGGEXPGNFSCN.LS DLOG(BGDGGEXPGNFSCN) = C(1) - C(2) * (LOG(BGDGGEXPGNFSCN(-1)) - LOG(BGDNYGDPMKTPCN(-1))) +DLOG(BGDNYGDPMKTPCN)'  C(3) * DLOG(BGDGGEXPGNFSCN(-1)) + (1 - C(3))* DLOG(BGDNYGDPMKTPCN) '+ C(10) * @DURING("1999")

		'TRANSFERS
			smpl %begin_date %end_estimate
				c = 0		
			'equation _BGDGGEXPTRNSCN.LS @PC(BGDGGEXPTRNSCN) = @PC(BGDNYGDPMKTPCN) + C(1) * @DURING("2015")
			equation _BGDGGEXPTRNSCN.LS DLOG(BGDGGEXPTRNSCN) = C(1) - C(2) * (LOG(BGDGGEXPTRNSCN(-1)) - LOG(BGDNYGDPMKTPCN(-1))) +DLOG(BGDNYGDPMKTPCN) '  C(3) * DLOG(BGDGGEXPTRNSCN(-1)) + (1 - C(3))* DLOG(BGDNYGDPMKTPCN) '+ C(10) * @DURING("1999")			
		'OTHER EXPENDITURES
			'Exogenous variables : BGDGGEXPOTHRCN
		...
		'CAPITAL EXPENDITURES
			...
			smpl 1992 %end_estimate
				c = 0
			equation _BGDGGEXPCAPTCN.LS DLOG(BGDGGEXPCAPTCN) = C(1) - C(2) * (LOG(BGDGGEXPCAPTCN(-1)) - LOG(BGDNYGDPMKTPCN(-1))) + DLOG(BGDNYGDPMKTPCN)' C(3) * DLOG(BGDGGEXPCAPTCN(-1)) + (1 - C(3))* DLOG(BGDNYGDPMKTPCN)
		endif
