...
hold.fill(o=2023)   3.6, 6.65041144303016, 7.50459956138896, 7.56402879919693, 9.16466324388323, 8.52177421318183, 7.9203856957653, 7.30956639018789
smpl @all
...
'Exclusion for row #20, Var:NEGDIFPRVKN
'Placing raw data from excel in hold variable for later treatment, and create the shock variable (_2)

hold.fill(o=2023)   1.7, 9.35786862649042, 9.99285924593434, 9.26519335942178, 11.2074071130929, 9.46522334821951, 8.32019259587467, 7.58555076573366
smpl @all
series BGDNEGDIFPRVKN_2 = BGDNEGDIFPRVKN
smpl %fcst_start %fcst_end

{%model}.exclude(r) BGDNEGDIFPRVKN_A

'Mode set to growth: setting shock value (_2) of series BGDNEGDIFPRVKN equal to level consistent with growth rates recorded in excel sheet (hold) 
series BGDNEGDIFPRVKN_2 = BGDNEGDIFPRVKN_2(-1) * (1 + hold / 100)
{%model}.override(m) BGDNEGDIFPRVKN
{%model}.exclude(m) BGDNEGDIFPRVKN
{%model}.exclude(r) BGDNEGDIFPRVKN_A



'Exclusion for row #26, Var:NEEXPGNFSKN
'Placing raw data from excel in hold variable for later treatment, and create the shock variable (_2)

hold.fill(o=2023)   10.6, 0.438502773012872, 1.59356237447561, 11.1643762938546, 5.86111792449153, 3.92870422760461, 2.77051017015455, 2.29009404763925
smpl @all
series BGDNEEXPGNFSKN_2 = BGDNEEXPGNFSKN
smpl %fcst_start %fcst_end

{%model}.exclude(r) BGDNEEXPGNFSKN_A

'Mode set to growth: setting shock value (_2) of series BGDNEEXPGNFSKN equal to level consistent with growth rates recorded in excel sheet (hold) 
series BGDNEEXPGNFSKN_2 = BGDNEEXPGNFSKN_2(-1) * (1 + hold / 100)
{%model}.override(m) BGDNEEXPGNFSKN
{%model}.exclude(m) BGDNEEXPGNFSKN
{%model}.exclude(r) BGDNEEXPGNFSKN_A



'Exclusion for row #31, Var:NEIMPGNFSKN
'Placing raw data from excel in hold variable for later treatment, and create the shock variable (_2)

hold.fill(o=2023)   -2.6, 5, 5, 5, 9.3258320760113, 7.11949297828325, 5.15365039988069, 3.1278684999452
smpl @all
series BGDNEIMPGNFSKN_2 = BGDNEIMPGNFSKN
smpl %fcst_start %fcst_end

{%model}.exclude(r) BGDNEIMPGNFSKN_A

'Mode set to growth: setting shock value (_2) of series BGDNEIMPGNFSKN equal to level consistent with growth rates recorded in excel sheet (hold) 
series BGDNEIMPGNFSKN_2 = BGDNEIMPGNFSKN_2(-1) * (1 + hold / 100)
{%model}.override(m) BGDNEIMPGNFSKN
{%model}.exclude(m) BGDNEIMPGNFSKN
{%model}.exclude(r) BGDNEIMPGNFSKN_A
'The following variables were in exogenous mode but were ucnhanged
{%model}.exclude(m) BGDNYGDPTFP BGDNEGDISTKBKN BGDNYGDPDISCKN
...
%Fname="C:\WBG\LocalITUtilities\BGD\Data\BGD2024-12-02~04-52-02soln.wf1"
wfsave(2) %Fname
close BGD2024-12-02~04-52-02soln
wfselect BGDsoln
