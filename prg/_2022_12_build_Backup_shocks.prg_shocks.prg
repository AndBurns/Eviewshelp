...
'============================
	...
	!changeMod = 1' If you want to add hardcore fiscal rule
...
' =================================================
	...
	scalar cpinit= 20*12 ' 1 USD = 12 RAND; base year is 2005 then *0.89 for EURO amount (41 for 40 euro UN) (11 for 10 euro)
	scalar cpgrowth= 4.5' 
	scalar cpgrowth1 = 0 ' 
' =================================================
...
'include generateTable.prg
	...
	if !changeMod = 1 then
		if (1=0) then
			...
			equation _{%cty}NYGDPFCSTXN.LS DLOG(ZAFNYGDPFCSTXN) = -0.4*(LOG(ZAFNYGDPFCSTXN(-1))-LOG(ZAFPSTAR(-1))) +0.5*DLOG(ZAFNYGDPFCSTXN(-1))+(1-0.5)*(0.5*ZAFINFLEXPT/100 + (1-0.5)*DLOG(ZAFPSTAR)) +0.4*ZAFNYGDPGAP_/100 + C(4)

			equation _ZAFNEIMPGSNEKN.LS DLOG(ZAFNEIMPGSNEKN) =-0.68*(LOG(ZAFNEIMPGSNEKN(-1))-LOG(ZAFNEGDETTOTKN(-1))+0.3*LOG(ZAFNEIMPGSNEXN(-1)/(ZAFNYGDPFCSTXN(-1))))+0.08*DLOG(ZAFNEIMPGSNEXN/ZAFNYGDPFCSTXN)+C(5)+1*DLOG(ZAFNEGDETTOTKN)+C(10)*@DURING("2009")

			equation _ZAFNEGDIFPRVXN.LS	DLOG(ZAFNEGDIFPRVXN) = C(1)*(LOG(ZAFNEGDIFPRVXN(-1))-LOG(ZAFNECONPRVTXN(-1))) + 1*DLOG(ZAFNECONPRVTXN) + C(10)
...
' ONLY CLIMATE (use this for fiscal revenues and transfers)
	...
	{%cty}GGREVCO2CKN_3 = cpinit 'Initial carbon price in RAND
...
' oil
	...
	group OIL2 ({%cty}ggbalovrlcn__4-{%cty}ggbalovrlcn__0) ({%cty}bncabfundcd_4/{%cty}nygdpmktpcd_4-{%cty}bncabfundcd_0/{%cty}nygdpmktpcd_0)*100
	...
	group EMET_BLOCK ZAFEMCOLFULT_4/1000000 ZAFEMOILFULT_4/1000000 ZAFEMGASFULT_4/1000000
	...
	smpl %sim_start-1 %sim_end-!cut
	group ene_int (1000*(zafnvcoleleqn_4/85.9845)+1000*(zafnvoileleqn_4/85.9845)+1000*(zafnvgaseleqn_4/85.9845)+1000*(zafnvreneleqn_4/85.9845))/@elem(ZAFNYGDPMKTPCD,"2011")  (1000*(zafnvcoleleqn_0/85.9845)+1000*(zafnvoileleqn_0/85.9845)+1000*(zafnvgaseleqn_0/85.9845)+1000*(zafnvreneleqn_0/85.9845))/@elem(ZAFNYGDPMKTPCD,"2011")  
	...
	group ene_int2 (ZAFCCEMISCO2Q_4/1000000)/((zafnvcolprodqn_4/85.9545)+(zafnvoilprodqn_4/85.9545)+(zafnvgasprodqn_4/85.9545)) (ZAFCCEMISCO2Q_0/1000000)/((zafnvcolprodqn_0/85.9545)+(zafnvoilprodqn_0/85.9545)+(zafnvgasprodqn_0/85.9545))  
