...
'Exclusion for row #10, Var:NECONPRVTXN
'Placing raw data from excel in hold variable for later treatment, and create the shock variable (_2)

hold.fill(o=2024)   6, 3.12167884739702, 4.3020370353825, 5.17820788355479, 6.05718044243855, 6.88506815668688, 7.67786711116436
smpl @all
series BGDNECONPRVTXN_2 = BGDNECONPRVTXN
smpl %fcst_start %fcst_end

{%model}.exclude(r) BGDNECONPRVTXN_A

'Mode set to growth: setting shock value (_2) of series BGDNECONPRVTXN equal to level consistent with growth rates recorded in excel sheet (hold) 
series BGDNECONPRVTXN_2 = BGDNECONPRVTXN_2(-1) * (1 + hold / 100)
{%model}.override(m) BGDNECONPRVTXN
{%model}.exclude(m) BGDNECONPRVTXN
{%model}.exclude(r) BGDNECONPRVTXN_A
...
'The following variables were in exogenous mode but were ucnhanged
{%model}.exclude(m) BGDFPCPITOTLXN BGDNECONGOVTXN BGDNEGDIFGOVXN BGDNEGDIFPRVXN BGDNEEXPGNFSXN BGDNEIMPGNFSXN BGDPANUSATLS BGDFMLBLPOLYFR BGDINFLEXPT
...
%Fname="C:\WBG\LocalITUtilities\BGD\Data\BGD2024-14-02~01-08-37soln.wf1"
wfsave(2) %Fname
close BGD2024-14-02~01-08-37soln
wfselect BGDsoln
