...
'Exclusion for row #19, Var:NEGDIFGOVXN
'Placing raw data from excel in hold variable for later treatment, and create the shock variable (_2)

hold.fill(o=2024)   4, 2.83061677762972, 3.77471406981942, 4.50540339599756, 5.25191705353476, 5.97144841885355, 6.67414063534382
smpl @all
series BGDNEGDIFGOVXN_2 = BGDNEGDIFGOVXN
smpl %fcst_start %fcst_end

{%model}.exclude(r) BGDNEGDIFGOVXN_A

'Mode set to growth: setting shock value (_2) of series BGDNEGDIFGOVXN equal to level consistent with growth rates recorded in excel sheet (hold) 
series BGDNEGDIFGOVXN_2 = BGDNEGDIFGOVXN_2(-1) * (1 + hold / 100)
{%model}.override(m) BGDNEGDIFGOVXN
{%model}.exclude(m) BGDNEGDIFGOVXN
{%model}.exclude(r) BGDNEGDIFGOVXN_A



'Exclusion for row #23, Var:NEGDIFPRVXN
'Placing raw data from excel in hold variable for later treatment, and create the shock variable (_2)

hold.fill(o=2024)   6, 3.70633314603599, 3.80201178801762, 4.03986995116972, 4.39149083198862, 4.82776546081294, 5.32719390490708
smpl @all
series BGDNEGDIFPRVXN_2 = BGDNEGDIFPRVXN
smpl %fcst_start %fcst_end

{%model}.exclude(r) BGDNEGDIFPRVXN_A

'Mode set to growth: setting shock value (_2) of series BGDNEGDIFPRVXN equal to level consistent with growth rates recorded in excel sheet (hold) 
series BGDNEGDIFPRVXN_2 = BGDNEGDIFPRVXN_2(-1) * (1 + hold / 100)
{%model}.override(m) BGDNEGDIFPRVXN
{%model}.exclude(m) BGDNEGDIFPRVXN
{%model}.exclude(r) BGDNEGDIFPRVXN_A
{%model}.exclude(r) BGDNEEXPGNFSXN_A



'Exclusion for row #32, Var:NEIMPGNFSXN
'Placing raw data from excel in hold variable for later treatment, and create the shock variable (_2)

hold.fill(o=2024)   4.6, 0.701377812767401, 3.55518735663782, 3.84114108833433, 4.19869264468695, 4.58486145094863, 4.97897718432321
smpl @all
series BGDNEIMPGNFSXN_2 = BGDNEIMPGNFSXN
smpl %fcst_start %fcst_end

{%model}.exclude(r) BGDNEIMPGNFSXN_A

'Mode set to growth: setting shock value (_2) of series BGDNEIMPGNFSXN equal to level consistent with growth rates recorded in excel sheet (hold) 
series BGDNEIMPGNFSXN_2 = BGDNEIMPGNFSXN_2(-1) * (1 + hold / 100)
{%model}.override(m) BGDNEIMPGNFSXN
{%model}.exclude(m) BGDNEIMPGNFSXN
{%model}.exclude(r) BGDNEIMPGNFSXN_A
{%model}.exclude(r) BGDPANUSATLS_A
{%model}.exclude(r) BGDFMLBLPOLYFR_A
'The following variables were in exogenous mode but were ucnhanged
{%model}.exclude(m) BGDFPCPITOTLXN BGDNECONPRVTXN BGDNECONGOVTXN BGDNEEXPGNFSXN BGDPANUSATLS BGDFMLBLPOLYFR BGDINFLEXPT
...
%Fname="C:\WBG\LocalITUtilities\BGD\Data\BGD2024-14-02~01-08-23soln.wf1"
wfsave(2) %Fname
close BGD2024-14-02~01-08-23soln
wfselect BGDsoln
