...
'Exclusion for row #6, Var:BXGSRMRCHCD
'Placing raw data from excel in hold variable for later treatment, and create the shock variable (_2)

hold.fill(o=2024)   0, 19.273389463181, 14.3887676343433, 11.0241244060077, 10.5989088826077, 10.6260234189165, 11.1339938642607
smpl @all
series BGDBXGSRMRCHCD_2 = BGDBXGSRMRCHCD
smpl %fcst_start %fcst_end

{%model}.exclude(r) BGDBXGSRMRCHCD_A

'Mode set to growth: setting shock value (_2) of series BGDBXGSRMRCHCD equal to level consistent with growth rates recorded in excel sheet (hold) 
series BGDBXGSRMRCHCD_2 = BGDBXGSRMRCHCD_2(-1) * (1 + hold / 100)
{%model}.override(m) BGDBXGSRMRCHCD
{%model}.exclude(m) BGDBXGSRMRCHCD
{%model}.exclude(r) BGDBXGSRMRCHCD_A
...
'The following variables were in exogenous mode but were ucnhanged
{%model}.exclude(m) BGDBMGSRMRCHCD BGDBXFSTREMTCD BGDBXFSTOTHRCD BGDBMFSTREMTCD BGDBMFSTOTHRCD BGDNYGDPMKTPCD BGDBFCAFFFDICD BGDBFCAFFPFTCD BGDBFCAFOOTHCD BGDBFCAFCAPTCD BGDBFCAFNEOMCD BGDBFCAFRACGCD BGDFIRESTOTLCD
...
%Fname="C:\WBG\LocalITUtilities\BGD\Data\BGD2024-14-02~01-19-06soln.wf1"
wfsave(2) %Fname
close BGD2024-14-02~01-19-06soln
wfselect BGDsoln
