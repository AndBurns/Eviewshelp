...
'Exclusion for row #40, Var:PANUSATLS_A
'Placing raw data from excel in hold variable for later treatment, and create the shock variable (_2)

hold.fill(o=2024)   20.8917960466818, 15.4313088822836, 7.6846860628179, 7.47579995648755, 7.45103050169375, 7.43386886446467, 7.42017094149143

MShold.fill(o=2024)   101.829409608963, 106.60952864475, 104.632382697826, 104.857582163824, 104.538511870721, 103.980364088256, 103.234619326095
series yg=mshold+hold
smpl @all
series BGDPANUSATLS_A_2 = BGDPANUSATLS_A
smpl %fcst_start %fcst_end

_BGDPANUSATLS.fit fit
...
smpl @all
series BGDPANUSATLS_A_2 = BGDPANUSATLS_A
series orig_AF = BGDPANUSATLS_A
series y = BGDPANUSATLS_0
smpl %fcst_start %solve_end
'
...
series orig_add_AF = BGDPANUSATLS_A 
series BGDPANUSATLS_A_S = BGDPANUSATLS_A
'Now caculate the additive add factor _A_S consistent with the desired growth path
smpl %fcst_start %solve_end
series BGDPANUSATLS_A_S = BGDPANUSATLS_A+Y(-1)*(delta_AF_add/100)
series BGDPANUSATLS_A_add = BGDPANUSATLS_A
...
test.override(m) BGDPANUSATLS_A
test.exclude(m)  BGDPANUSATLS_A
...
smpl %solve_start %solve_end
series BGDPANUSATLS = BGDPANUSATLS_s
series BGDPANUSATLS_A = orig_AF


'Calculate new intercept-type add factor
smpl %solve_start %solve_end
{%model}.addinit(v=n,s=a) BGDPANUSATLS
...
series BGDPANUSATLS_A_2 = BGDPANUSATLS_A
series BGDPANUSATLS_A = orig_AF
series BGDPANUSATLS= orig_ser
{%model}.override(m) BGDPANUSATLS_A
{%model}.exclude(m) BGDPANUSATLS_A
{%model}.exclude(r) BGDFMLBLPOLYFR
'The following variables were in exogenous mode but were ucnhanged
{%model}.exclude(m) BGDFPCPITOTLXN_A BGDNECONPRVTXN_A BGDNECONGOVTXN_A BGDNEGDIFGOVXN_A BGDNEGDIFPRVXN_A BGDNEEXPGNFSXN_A BGDNEIMPGNFSXN_A BGDFMLBLPOLYFR_A BGDINFLEXPT
...
%Fname="C:\WBG\LocalITUtilities\BGD\Data\BGD2024-15-02~03-52-27soln.wf1"
wfsave(2) %Fname
close BGD2024-15-02~03-52-27soln
wfselect BGDsoln
