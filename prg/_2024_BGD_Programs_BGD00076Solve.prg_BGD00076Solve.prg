...
MShold.fill(o=2024)   264.601609629099, 208.988009064493, 112.674001585673, 60.663435284473, 44.2546195912236, 61.6248764644878, 120.330951937863
...
smpl %fcst_start %solve_end
series BGDBFCAFRACGCD_A_S = BGDBFCAFRACGCD_A+Y(-1)*(delta_AF_add/100)
series BGDBFCAFRACGCD_A_add = BGDBFCAFRACGCD_A
...
'Exclusion for row #69, Var:FIRESTOTLCD_A
'Placing raw data from excel in hold variable for later treatment, and create the shock variable (_2)

hold.fill(o=2024)   0, 0, 0, 0, 0, 0, 0

MShold.fill(o=2024)   -45.2409784736519, 53.6313667647435, 18.9369900322107, 11.9503094663509, 8.53733308606506, 6.32074915731107, 4.55685957703953
series yg=mshold+hold
smpl @all
series BGDFIRESTOTLCD_A_2 = BGDFIRESTOTLCD_A
smpl %fcst_start %fcst_end

_BGDFIRESTOTLCD.fit fit
...
smpl @all
series BGDFIRESTOTLCD_A_2 = BGDFIRESTOTLCD_A
series orig_AF = BGDFIRESTOTLCD_A
series y = BGDFIRESTOTLCD_0
smpl %fcst_start %solve_end
'
'Calculate value of dependent variable that we want to reach using excelified growth add factor
smpl %fcst_start %solve_end
...
smpl @all
series orig_AF = BGDFIRESTOTLCD_A 
series  orig_ser = BGDFIRESTOTLCD_0

'Create temporary model comprised of the equation being AF'd
delete(noerr) test
model test
test.merge _BGDFIRESTOTLCD
...
smpl @all
'Save the AF consistent with the additive model and create a new one _S for the simul
series orig_add_AF = BGDFIRESTOTLCD_A 
series BGDFIRESTOTLCD_A_S = BGDFIRESTOTLCD_A
'Now caculate the additive add factor _A_S consistent with the desired growth path
smpl %fcst_start %solve_end
series BGDFIRESTOTLCD_A_S = BGDFIRESTOTLCD_A+Y(-1)*(delta_AF_add/100)
series BGDFIRESTOTLCD_A_add = BGDFIRESTOTLCD_A
'Solve the additive model using the AF shock -- in order to find the pathway for LHS variable
setmaxerrs 2
test.scenario(d) "Find New Y"
seterrcount 0
...
test.scenario(n, a = s, i = "baseline", c) "Find New Y"
test.scenario "Find New Y"
test.override(m) BGDFIRESTOTLCD_A
test.exclude(m)  BGDFIRESTOTLCD_A
...
'Have calculated the new path for the LHS variable
smpl %solve_start %solve_end
series BGDFIRESTOTLCD = BGDFIRESTOTLCD_s
series BGDFIRESTOTLCD_A = orig_AF


'Calculate new intercept-type add factor
smpl %solve_start %solve_end
{%model}.addinit(v=n,s=a) BGDFIRESTOTLCD

'Set shock veriosn of AF (_2) equal to the level of the AF consistent with new growth path and then restore old values of _A and root series
series BGDFIRESTOTLCD_A_2 = BGDFIRESTOTLCD_A
series BGDFIRESTOTLCD_A = orig_AF
series BGDFIRESTOTLCD= orig_ser
{%model}.override(m) BGDFIRESTOTLCD_A
{%model}.exclude(m) BGDFIRESTOTLCD_A
'The following variables were in exogenous mode but were ucnhanged
{%model}.exclude(m) BGDBXGSRMRCHCD BGDBMGSRMRCHCD BGDBXFSTREMTCD BGDBXFSTOTHRCD BGDBMFSTREMTCD BGDBMFSTOTHRCD BGDNYGDPMKTPCD BGDBFCAFFFDICD BGDBFCAFFPFTCD BGDBFCAFOOTHCD BGDBFCAFCAPTCD BGDBFCAFNEOMCD
...
%Fname="C:\WBG\LocalITUtilities\BGD\Data\BGD2024-13-02~08-31-01soln.wf1"
wfsave(2) %Fname
close BGD2024-13-02~08-31-01soln
wfselect BGDsoln
