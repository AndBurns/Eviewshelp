...
'*******************************************************
	...
	c = 0
	equation _BGDNEGDIFPRVKN.LS DLOG(BGDNEGDIFPRVKN) =  -0.2* (LOG(BGDNEGDIFPRVKN(-1)) - LOG(1-BGDNYYWBTOTLCN_(-1)) - LOG(BGDNYGDPPOTLKN(-1)) + LOG(BGDNEKRTTOTLXN(-1)/BGDNYGDPFCSTXN(-1))- C(1)) +c(10)* DLOG(BGDNYGDPMKTPKN) - 0.15 * DLOG(BGDNEKRTTOTLXN/BGDNEGDIFPRVXN)  
	'Exports (behavioral equation)
	smpl 1991 %end_estimate
	c = 0
	equation _BGDNEEXPGNFSKN.ls DLOG(BGDNEEXPGNFSKN) =  -0.3* (LOG(BGDNEEXPGNFSKN(-1)) - LOG(BGDXMKT(-1)) -C(1)*@during("2010 2019") + c(4)*@during("1991 2009" ) - 0.6 *LOG(BGDNEEXPGNFSXN(-1)/(BGDNYGDPFCSTXN(-1)))) + 0.6 * DLOG(BGDNEEXPGNFSXN/(BGDNYGDPFCSTXN)) +1 * DLOG(BGDXMKT)
	'Imports (behavioural equation)
	smpl 1990 %end_estimate
	c = 0
	equation _BGDNEIMPGNFSKN.LS DLOG(BGDNEIMPGNFSKN) =-0.3*(LOG(BGDNEIMPGNFSKN(-1))-LOG(BGDNEGDETTOTKN(-1))+0.6*LOG(BGDNEIMPGNFSXN(-1)/(BGDNYGDPFCSTXN(-1))))-0.4*DLOG(BGDNEIMPGNFSXN/BGDNYGDPFCSTXN)+C(5)+0.8*DLOG(BGDNYGDPPOTLKN)+C(10)*@DURING("2009") 
...
'*******************************************************
	...
	smpl %begin_date %end_estimate
	c = 0
	equation _BGDNEGDIFGOVXN.LS(OPTMETHOD=LEGACY) DLOG(BGDNEGDIFGOVXN) = C(1) - C(2) * (LOG(BGDNEGDIFGOVXN(-1)) - LOG(BGDNECONPRVTXN(-1))) + 1* DLOG(BGDNECONPRVTXN)+C(10)*@DURING("1987")+C(11)*@DURING("1997")+C(12)*@DURING("2000")
	'Private investment deflator (behavioural equation)
	smpl 2000 %end_estimate
	c = 0
	equation _BGDNEGDIFPRVXN.LS DLOG(BGDNEGDIFPRVXN) = C(1) -0.4* (LOG(BGDNEGDIFPRVXN(-1)) - LOG(BGDNECONPRVTXN(-1))) + 1 * DLOG(BGDNECONPRVTXN)  + C(10) * @DURING("2008")
	'Export deflator (behavioural equation)
	smpl %begin_date %end_estimate
	c = 0
	equation _BGDNEEXPGNFSXN.LS DLOG(BGDNEEXPGNFSXN) = C(1) -0.3* (LOG(BGDNEEXPGNFSXN(-1)) - 0.84 * LOG((1+BGDGGREVEXPDER(-1)/100)*BGDNYGDPFCSTXN(-1)) - (1-0.84) * LOG((1+BGDGGREVEXPDER(-1)/100)*BGDPXKEY(-1)*BGDPANUSATLS(-1))) + 0.83 * DLOG((1+BGDGGREVEXPDER/100)*BGDNYGDPFCSTXN) + (1-0.83) * DLOG((1+BGDGGREVEXPDER(-1)/100)*BGDPXKEY*BGDPANUSATLS)+ C(10) * @DURING("2004")+C(11)*@DURING("2005 2018")
...
'*******************************************************
	...
	c=0
	smpl %begin_date %end_estimate
	equation _BGDNYWRTTOTLXN.LS DLOG(BGDNYWRTTOTLXN) = (C(1)*BGDINFLEXPT/100+(1-C(1))*DLOG(BGDNYGDPFCSTXN(-1))+DLOG(BGDNYGDPPOTLKN/BGDLMEMPSTRL)) - 0.1*(BGDLMUNRTOTL_-BGDLMUNRSTRL_)/100
	' Employment
	 c=0
	smpl %begin_date %end_estimate
	equation _BGDLMEMPTOTL.LS DLOG(BGDLMEMPTOTL) =-0.2*(LOG(BGDLMEMPTOTL(-1))-LOG(BGDLMEMPSTRL(-1))) + DLOG(BGDLMEMPSTRL) - 0.1*(DLOG(BGDNYWRTTOTLXN/BGDNYGDPFCSTXN)-DLOG(BGDNYGDPPOTLKN/BGDLMEMPSTRL)) +0.2*(DLOG(BGDNEGDETTOTKN)-DLOG(BGDNYGDPPOTLKN))+C(4)
	' Participation rate
	c=0
	smpl %begin_date %end_estimate
...
'*******************************************************
	...
	smpl 1986 %end_estimate
	c = 0
	equation _BGDNYGDPFCSTXN.LS DLOG(BGDNYGDPFCSTXN) = -0.2*(LOG(BGDNYGDPFCSTXN(-1))-LOG(BGDPSTAR(-1)))+C(2)+C(3)*DLOG(BGDNYGDPFCSTXN(-1))+(1-C(3))*(0.7*(BGDINFLEXPT/100)+(1-0.7)*DLOG(BGDPSTAR(-1))) +0.35*BGDNYGDPGAP_/100
...
'*******************************************************			
		...
		if !fiscRule = 1 then
			...
			smpl %begin_date %end_estimate
			c = 0
			equation _BGDGGEXPWAGECN.LS DLOG(BGDGGEXPWAGECN) = C(1) - C(2) * (LOG(BGDGGEXPWAGECN(-1)) - LOG(BGDNYGDPMKTPCN(-1))) +DLOG(BGDNYGDPMKTPCN) '			
			'GOODS and SERVICES
			smpl 2000 %end_estimate
			c = 0		
			equation _BGDGGEXPGNFSCN.LS DLOG(BGDGGEXPGNFSCN) = C(1) - C(2) * (LOG(BGDGGEXPGNFSCN(-1)) - LOG(BGDNYGDPMKTPCN(-1))) +DLOG(BGDNYGDPMKTPCN)
			'TRANSFERS
			smpl %begin_date %end_estimate
			c = 0		
			equation _BGDGGEXPTRNSCN.LS DLOG(BGDGGEXPTRNSCN) = C(1) - C(2) * (LOG(BGDGGEXPTRNSCN(-1)) - LOG(BGDNYGDPMKTPCN(-1))) +DLOG(BGDNYGDPMKTPCN) 
			'OTHER EXPENDITURES
			'Exogenous variables : BGDGGEXPOTHRCN	
			'CAPITAL EXPENDITURES
			smpl 1992 %end_estimate
			c = 0
			equation _BGDGGEXPCAPTCN.LS DLOG(BGDGGEXPCAPTCN) = C(1) - C(2) * (LOG(BGDGGEXPCAPTCN(-1)) - LOG(BGDNYGDPMKTPCN(-1))) + DLOG(BGDNYGDPMKTPCN)
		endif
...
'*******************************************************		
			...
			BGD.append @identity BGDBFCAFFINXCD = BGDBFCAFFFDICD + BGDBFCAFFPFTCD + BGDBFCAFOTHRCD
				...
				c = 0
				equation _BGD.LS @PC(BGDBFCAFFPFTCD) = @PC(BGDNYGDPMKTPCD) + C(1) * @DURING("2015")
