...
'Exclusion for row #9, Var:GGREVDRCTCN_A
'Placing raw data from excel in hold variable for later treatment, and create the shock variable (_2)

hold.fill(o=2023)   0, 0, 0, 0, 0, 0, 0, 0

MShold.fill(o=2023)   12.7420911136895, 13.5027230976592, 13.9748223547248, 13.7966175901892, 13.9621684281262, 14.1108780915801, 14.1870877774447, 14.1951576568256
series yg=mshold+hold
smpl @all
series BGDGGREVDRCTCN_A_2 = BGDGGREVDRCTCN_A
smpl %fcst_start %fcst_end

_BGDGGREVDRCTCN.fit fit
...
smpl @all
series BGDGGREVDRCTCN_A_2 = BGDGGREVDRCTCN_A
series orig_AF = BGDGGREVDRCTCN_A
series y = BGDGGREVDRCTCN_0
smpl %fcst_start %solve_end
'
...
series orig_add_AF = BGDGGREVDRCTCN_A 
series BGDGGREVDRCTCN_A_S = BGDGGREVDRCTCN_A
'Now caculate the additive add factor _A_S consistent with the desired growth path
smpl %fcst_start %solve_end
series BGDGGREVDRCTCN_A_S = BGDGGREVDRCTCN_A+Y(-1)*(delta_AF_add/100)
series BGDGGREVDRCTCN_A_add = BGDGGREVDRCTCN_A
...
test.override(m) BGDGGREVDRCTCN_A
test.exclude(m)  BGDGGREVDRCTCN_A
...
smpl %solve_start %solve_end
series BGDGGREVDRCTCN = BGDGGREVDRCTCN_s
series BGDGGREVDRCTCN_A = orig_AF


'Calculate new intercept-type add factor
smpl %solve_start %solve_end
{%model}.addinit(v=n,s=a) BGDGGREVDRCTCN
...
series BGDGGREVDRCTCN_A_2 = BGDGGREVDRCTCN_A
series BGDGGREVDRCTCN_A = orig_AF
series BGDGGREVDRCTCN= orig_ser
{%model}.override(m) BGDGGREVDRCTCN_A
{%model}.exclude(m) BGDGGREVDRCTCN_A
...
MShold.fill(o=2023)   11.4692019897727, 12.1849205268278, 12.7828307235211, 13.5734495411642, 13.8677478027489, 13.98641337916, 13.9774627771071, 13.9065108078684
series yg=mshold+hold
smpl @all
...
MShold.fill(o=2023)   19.1585062596602, 15.1209766932644, 12.799436787282, 14.1718923415779, 13.1586968110397, 12.4730199966797, 12.0781511616203, 11.903419371036
series yg=mshold+hold
smpl @all
...
MShold.fill(o=2023)   22.745308846868, 18.3421679781867, 16.2538884879673, 15.5062654910268, 14.4838614872729, 13.6849647330248, 12.9945351818462, 12.3871934177393
series yg=mshold+hold
smpl @all
...
MShold.fill(o=2023)   11.4692019897727, 12.1849205268306, 12.7828307239372, 13.5734495414439, 13.8677478031909, 13.9864133794398, 13.9774627773147, 13.906510807871
series yg=mshold+hold
smpl @all
...
MShold.fill(o=2023)   11.4692019897727, 12.1849205268283, 12.7828307245371, 13.5734495420658, 13.8677478039488, 13.986413380029, 13.9774627777948, 13.9065108078703
series yg=mshold+hold
smpl @all
...
MShold.fill(o=2023)   12.7420911136895, 13.5027230962058, 13.9748213902743, 13.7966169671407, 13.962166602024, 14.1108766818491, 14.1870864548614, 14.1951577724314
series yg=mshold+hold
smpl @all
...
MShold.fill(o=2023)   12.7420911136895, 13.5027230962057, 13.9748213902743, 13.7966169671407, 13.9621666020241, 14.1108766818491, 14.1870864548614, 14.1951577724314
series yg=mshold+hold
smpl @all
...
'Exclusion for row #49, Var:GGREVTOTHCN_A
'Placing raw data from excel in hold variable for later treatment, and create the shock variable (_2)

hold.fill(o=2023)   , 0, 0, 0, 0, 0, 0, 0

MShold.fill(o=2023)   , 13.5027230962057, 13.9748213902743, 13.7966169671407, 13.962166602024, 14.1108766818491, 14.1870864548614, 14.1951577724314
series yg=mshold+hold
smpl @all
series BGDGGREVTOTHCN_A_2 = BGDGGREVTOTHCN_A
smpl %fcst_start %fcst_end

_BGDGGREVTOTHCN.fit fit
...
smpl @all
series BGDGGREVTOTHCN_A_2 = BGDGGREVTOTHCN_A
series orig_AF = BGDGGREVTOTHCN_A
series y = BGDGGREVTOTHCN_0
smpl %fcst_start %solve_end
'
'Calculate value of dependent variable that we want to reach using excelified growth add factor
smpl %fcst_start %solve_end
...
smpl @all
series orig_AF = BGDGGREVTOTHCN_A 
series  orig_ser = BGDGGREVTOTHCN_0

'Create temporary model comprised of the equation being AF'd
delete(noerr) test
model test
test.merge _BGDGGREVTOTHCN
...
smpl @all
'Save the AF consistent with the additive model and create a new one _S for the simul
series orig_add_AF = BGDGGREVTOTHCN_A 
series BGDGGREVTOTHCN_A_S = BGDGGREVTOTHCN_A
'Now caculate the additive add factor _A_S consistent with the desired growth path
smpl %fcst_start %solve_end
series BGDGGREVTOTHCN_A_S = BGDGGREVTOTHCN_A+Y(-1)*(delta_AF_add/100)
series BGDGGREVTOTHCN_A_add = BGDGGREVTOTHCN_A
'Solve the additive model using the AF shock -- in order to find the pathway for LHS variable
setmaxerrs 2
test.scenario(d) "Find New Y"
seterrcount 0
...
test.scenario(n, a = s, i = "baseline", c) "Find New Y"
test.scenario "Find New Y"
test.override(m) BGDGGREVTOTHCN_A
test.exclude(m)  BGDGGREVTOTHCN_A
...
'Have calculated the new path for the LHS variable
smpl %solve_start %solve_end
series BGDGGREVTOTHCN = BGDGGREVTOTHCN_s
series BGDGGREVTOTHCN_A = orig_AF


'Calculate new intercept-type add factor
smpl %solve_start %solve_end
{%model}.addinit(v=n,s=a) BGDGGREVTOTHCN

'Set shock veriosn of AF (_2) equal to the level of the AF consistent with new growth path and then restore old values of _A and root series
series BGDGGREVTOTHCN_A_2 = BGDGGREVTOTHCN_A
series BGDGGREVTOTHCN_A = orig_AF
series BGDGGREVTOTHCN= orig_ser
{%model}.override(m) BGDGGREVTOTHCN_A
{%model}.exclude(m) BGDGGREVTOTHCN_A
{%model}.exclude(r) BGDGGREVNONTCN
{%model}.exclude(r) BGDGGREVGRNTCN
'The following variables were in exogenous mode but were ucnhanged
{%model}.exclude(m) BGDGGREVOTHRCN BGDGGREVNONTCN_A BGDGGREVGRNTCN_A
...
%Fname="C:\WBG\LocalITUtilities\BGD\Data\BGD2024-05-02~10-57-52soln.wf1"
wfsave(2) %Fname
close BGD2024-05-02~10-57-52soln
wfselect BGDsoln
