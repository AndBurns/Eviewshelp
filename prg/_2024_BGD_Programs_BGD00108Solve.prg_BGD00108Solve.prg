...
'Exclusion for row #38, Var:PANUSATLS
'Placing raw data from excel in hold variable for later treatment, and create the shock variable (_2)

hold.fill(o=2024)   9.99999999999999, 0, 0.500269470886039, -0.0541823436176925, -0.431693158171431, -0.672903486676946, -0.837772545013282
smpl @all
series BGDPANUSATLS_2 = BGDPANUSATLS
smpl %fcst_start %fcst_end

{%model}.exclude(r) BGDPANUSATLS_A

'Mode set to growth: setting shock value (_2) of series BGDPANUSATLS equal to level consistent with growth rates recorded in excel sheet (hold) 
series BGDPANUSATLS_2 = BGDPANUSATLS_2(-1) * (1 + hold / 100)
{%model}.override(m) BGDPANUSATLS
{%model}.exclude(m) BGDPANUSATLS
{%model}.exclude(r) BGDPANUSATLS_A
{%model}.exclude(r) BGDFMLBLPOLYFR_A
'The following variables were in exogenous mode but were ucnhanged
{%model}.exclude(m) BGDFPCPITOTLXN BGDNECONPRVTXN BGDNECONGOVTXN BGDNEGDIFGOVXN BGDNEGDIFPRVXN BGDNEEXPGNFSXN BGDNEIMPGNFSXN BGDFMLBLPOLYFR BGDINFLEXPT
...
%Fname="C:\WBG\LocalITUtilities\BGD\Data\BGD2024-14-02~01-12-24soln.wf1"
wfsave(2) %Fname
close BGD2024-14-02~01-12-24soln
wfselect BGDsoln
