// Copyright (c) 2002-2017 Quadralay Corporation.  All rights reserved.
//
...
function  HTMLHelp_Object()
{
  ...
  this.fExternalFileLink       = HTMLHelp_ExternalFileLink;
}
...
function HTMLHelp_ExternalFileLink(ParamCHMRelativeLinkURI, ParamSplitRelativeURI, ParamTargetWindow)
{
  'use strict';

  var VarColonIndex, VarBackslashIndex, VarTargetURI;

  // CHM context?
  //
  if ((window.location.href.toLowerCase().indexOf('ms-its:') === 0) || (window.location.href.toLowerCase().indexOf('mk:') === 0))
  {
    // Locate end of CHM protocol indicator ('ms-its:' or 'mk:@MSITStore:')
    //
    VarColonIndex = window.location.href.indexOf(':');
    if (VarColonIndex < 6) {
      VarColonIndex = window.location.href.indexOf(':', VarColonIndex + 1);
    }

    // Locate last component before CHM (ms-its:C:\dir\file.chm::/topic.htm)
    //
    VarBackslashIndex = window.location.href.lastIndexOf('\\');

    // Build absolute path to CHM directory
    //
    VarTargetURI = 'file:///' + window.location.href.substring(VarColonIndex + 1, VarBackslashIndex + 1);
    VarTargetURI += ParamCHMRelativeLinkURI;
    VarBackslashIndex = VarTargetURI.indexOf('\\')
    while (VarBackslashIndex >= 0)
    {      
      VarTargetURI = VarTargetURI.substring(0, VarBackslashIndex) + '/' + VarTargetURI.substring(VarBackslashIndex + 1);
  ...
  {
    VarTargetURI = ParamSplitRelativeURI;
  }

  // Open location
  //
  window.open(VarTargetURI, ParamTargetWindow);
}
